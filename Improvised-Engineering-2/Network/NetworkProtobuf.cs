...
namespace Improvised_Engineering_2
{
    ...
    public class NetworkProtobuf
    {
        ...
        /// </summary>
        public NetworkProtobuf(ushort channelId)
        {
            ChannelId = channelId;
        }
        ...
        private void ReceivedPacket(ushort handlerID, byte[] rawData, ulong steamID, bool fromServer) // executed when a packet is received on this machine
        {
            try
            {
            ...
            catch (Exception e)
            {
                ...
                Debug.Notify($"[ERROR: {GetType().FullName}: {e.Message} | Send SpaceEngineers.Log to mod author]", 5000, "Red", true);
            }
        }
